...
[
    ...
    get_list/2,
    get_ets_info/2,
    get_goods_list_from_ets/2,
    make_info/2,
    get_goods_name/1,
    get_task_mon/1,
    get_task_goods_num/2,
    transform_goods_type_to_goods/1,
    get_current_equip/2,
    count_current_equip_by_list/2,
    count_current_equip_by_info/2,
    get_goods_from_ets/1,
    get_goods_type_by_id/1,
    get_add_goods/5,
    get_goods_from_db/1,
    get_goods_by_cell/3,
    get_goods_list_by_location/2,
    get_goods_list_by_type_id/4,
    get_goods_list_by_type_id/3,
    get_mend_goods_list/2,
    get_goods_list_by_type_and_location/3,
    get_shop_goods_list/2,
    get_shop_info/2,
    get_add_goods_attribute/4,
    get_attribute_by_goods_and_player/3,
    ...
    count_equip_suit_list/2,
    get_suit_num/2,
    update_equip_suit_list/3,
    ...
    get_goods_max_use_num/1,
    get_goods_attrition/2,
    get_mend_cost/2,
    get_equip_cell_by_subtype/1,
    get_null_cell_num/3,
    get_null_cells/2,
    get_goods_exp/1,
    sort/2,
...
init_goods(PlayerId) ->
    ...
    Sql = io_lib:format(<<
        "select id, player_id, goods_id, type, subtype, equip_type, price_type, price, sell_price, bind,
        trade, sell, isdrop, level, vitality, spirit, hp, mp, forza, agile, wit, att, def, hit, dodge,
        crit, ten, speed, attrition, use_num, suit_id, quality, quality_his, quality_fail, stren, stren_his,
        stren_fail, hole, hole1_goods, hole2_goods, hole3_goods, location, cell, num, color,
        expire_time from `goods` where player_id = ~p and location <> 5 ">>, [PlayerId]),
...
init_goods_attribute(PlayerId) ->
    ...
    Sql = io_lib:format(<<"select id, player_id, gid, attribute_type, attribute_id, value_type, hp, mp, att, def, hit,
    dodge, crit, ten from `goods_attribute` where player_id = ~p ">>, [PlayerId]),
    case db_sql:get_all(Sql) of
        [] -> skip;
        AttributeList when is_list(AttributeList) ->
            lists:foreach(F, AttributeList);
...
get_goods_list_from_ets(Tab, Pattern) ->
...
get_task_goods_num(PlayerId, GoodsTypeId) ->
    ...
    GoodsList = get_goods_list_from_ets(?ETS_GOODS_ONLINE, Pattern),
    get_goods_total_num(GoodsList).

transform_goods_type_to_goods(GoodsTypeInfo) ->
    #goods{
        ...
        use_num = get_goods_max_use_num(GoodsTypeInfo#ets_goods_type.attrition),
...
get_current_equip(GoodsStatus, Equip) ->
    EquipList = goods_util:get_goods_list_by_type_and_location(GoodsStatus#goods_status.player_id, 10, Equip),
    MountList = goods_util:get_goods_list_by_type_and_location(GoodsStatus#goods_status.player_id, 31, 1),
    GoodsList = EquipList ++ MountList,
    [NewStatus, _] = count_current_equip_by_list(GoodsList, [GoodsStatus, on]),
    NewStatus.

count_current_equip_by_list(GoodsList, [GoodsStatus, Type]) ->
    lists:foldl(fun count_current_equip_by_info/2, [GoodsStatus, Type], GoodsList).

count_current_equip_by_info(GoodsInfo, [GoodsStatus, Type]) ->
...
get_goods_type_by_id(GoodsTypeId) ->
...
%% @spec get_goods_from_db(GoodsId) -> record()
get_goods_from_db(GoodsId) ->
...
%% 取物品列表
get_goods_list_by_location(PlayerId, Location) ->
    case Location =/= 5 of
        true ->
            Pattern = #goods{player_id = PlayerId, location = Location, _ = '_'},
            get_goods_list_from_ets(?ETS_GOODS_ONLINE, Pattern);
        false -> %% 仓库
...
%% 获取同类物品列表
get_goods_list_by_type_id(PlayerId, GoodsTypeId, Bind, Location) ->
    case Location =/= 5 of
        true ->
            Pattern = #goods{player_id = PlayerId, goods_id = GoodsTypeId, bind = Bind, location = Location, _ = '_'},
            get_goods_list_from_ets(?ETS_GOODS_ONLINE, Pattern);
        false -> %% 仓库
            Sql = io_lib:format(<<"select id, player_id, goods_id, type, subtype, equip_type, price_type, price, sell_price, bind, trade, sell,
            isdrop, level, vitality, spirit, hp, mp, forza, agile, wit, att, def, hit, dodge, crit, ten, speed, attrition, use_num, suit_id,
            quality, quality_his, quality_fail, stren, stren_his, stren_fail, hole, hole1_goods, hole2_goods, hole3_goods, location, cell,
            num, color, expire_time from `goods` where player_id = ~p and goods_id = ~p and bind = ~p and location = 5 ">>,
                [PlayerId, GoodsTypeId, Bind]),
            get_list(goods, Sql)
    end.
get_goods_list_by_type_id(PlayerId, GoodsTypeId, Location) ->
    case Location =/= 5 of
        true ->
...
%% 查找有耐久度的装备
get_mend_goods_list(PlayerId, EquipLocal) ->
    L1 = get_goods_list_by_type_and_location(PlayerId, 10, EquipLocal),
    L2 = get_goods_list_by_type_and_location(PlayerId, 10, ?GOODS_LOCATION_BAG),
    L1 ++ L2.

%% 取装备列表
get_goods_list_by_type_and_location(PlayerId, Type, Location) ->
    Pattern = #goods{player_id = PlayerId, type = Type, location = Location, _ = '_'},
    get_goods_list_from_ets(?ETS_GOODS_ONLINE, Pattern).

%% 取商店物品列表
get_shop_goods_list(ShopType, ShopSubtype) ->
    ...
    get_goods_list_from_ets(?ETS_SHOP, Pattern).
...
%% 取装备属性信息
get_attribute_by_goods_and_player(PlayerId, GoodsId, AttributeType) ->
...
get_goods_attribute_list(PlayerId, GoodsId, AttributeType) ->
    Pattern = #goods_attribute{player_id = PlayerId, gid = GoodsId, attribute_type = AttributeType, _ = '_'},
    get_goods_list_from_ets(?ETS_GOODS_ATTRIBUTE, Pattern).
...
get_suit_attribute_list(SuitId, SuitNum) ->
    ...
    SuitAttributeList = get_goods_list_from_ets(?ETS_SUIT_ATTRIBUTE, Pattern),
...
%% 取装备的属性加成
%% @return [Hp, Mp, Att, Def, Hit, Dodge, Crit, Ten]
get_equip_attribute(PlayerId, EquipLocal, EquipSuitList) ->
    EquipList = get_goods_list_by_type_and_location(PlayerId, 10, EquipLocal),
    %% 装备属性加成
    [Hp, Mp, Att, Def, Hit, Dodge, Crit, Ten] = lists:foldl(fun count_goods_effect/2, [0, 0, 0, 0, 0, 0, 0, 0], EquipList),
    %% 装备套装属性加成
    [Hp1, Mp1, Att1, Def1, Hit1, Dodge1, Crit1, Ten1] = get_suit_attribute(EquipSuitList),
...
%% 计算玩家的套装属性
%% @param PlayerId::int, EquipLocal::int
%% @return EquipSuitList::List
count_equip_suit_list(PlayerId, EquipLocal) ->
    EquipList = goods_util:get_goods_list_by_type_and_location(PlayerId, 10, EquipLocal),
    F = fun(GoodsInfo, EquipSuit) ->
        update_equip_suit_list(EquipSuit, 0, GoodsInfo#goods.suit_id)
        end,
    EquipSuitList = lists:foldl(F, [], EquipList),
    EquipSuitList.
...
get_suit_num(EquipSuitList, SuitId) ->
    case SuitId > 0 of
        true ->
            case lists:keyfind(SuitId, 1, EquipSuitList) of
                false -> 0;
...
%% 更新套裝列表
%% @param EquipSuitList::[
update_equip_suit_list(EquipSuitList, OldSuitId, NewSuitId) ->
    %% 删除
    EquipSuitList1 = if OldSuitId > 0 ->
        case lists:keyfind(OldSuitId, 1, EquipSuitList) of
            false ->
                EquipSuitList;
            {OldSuitId, SuitNum} when SuitNum > 1 ->
                lists:keyreplace(OldSuitId, 1, EquipSuitList, {OldSuitId, SuitNum - 1});
            {OldSuitId, _} ->
                lists:keydelete(OldSuitId, 1, EquipSuitList)
        end;
                         true ->
                             EquipSuitList
                     end,
    %% 添加
    EquipSuitList2 = if NewSuitId > 0 ->
        case lists:keyfind(NewSuitId, 1, EquipSuitList1) of
            false ->
                [{NewSuitId, 1} | EquipSuitList1];
            {NewSuitId, SuitNum2} ->
                lists:keyreplace(NewSuitId, 1, EquipSuitList1, {NewSuitId, SuitNum2 + 1})
        end;
                         true -> EquipSuitList1
                     end,
    EquipSuitList2.
...
%% 取得装备的使用次数
%% @spec get_goods_max_use_num(Attrition) -> UseNum
get_goods_max_use_num(Attrition) ->
    Attrition * 10.
...
%% 检查物品还需要多少格子数
%% @param MaxNum::int 物品最大堆叠数
%% @param GoodsNum::int 新加入的物品数量
...
get_null_cells(PlayerId, CellNum) ->
    ...
    List = get_goods_list_from_ets(?ETS_GOODS_ONLINE, Pattern),
...
can_equip(PlayerStatus, GoodsTypeId, Cell) ->
    GoodsTypeInfo = get_goods_type_by_id(GoodsTypeId),
    DefCell = get_equip_cell_by_subtype(GoodsTypeInfo#ets_goods_type.subtype),
...
%% 这个方法要求F(N, Data)返回的N1必须比N小，否则会死循环
deeploop(F, N, Data) ->
    case N > 0 of
        true ->
            [N1, Data1] = F(N, Data),
