...
load_guild_into_ets([Id,Name,Tenet,Announce,InitiatorId,InitiatorName,ChiefId,ChiefName,DeputyChief1Id,DeputyChief1Name,
    DeputyChief2Id,DeputyChief2Name,DeputyChiefNum,MemberNum,MemberCapacity,Realm,Level,Reputation,Funds,Contribution,
    ContributionDaily,ContributionThreshold,ContributionGetNextTime,CombatNum,CombatVictoryNum,QQ,CreateTime,DisbandFlag,
    DisbandConfirmTime,DisbandDeadlineTime]) ->
    % 1- 收取日建设
    NowTime = util:unixtime(),
    [State, ContributionNew, ContributionGetNextTimeNew, ContributionDailyNew, ContributionThresholdNew, LevelNew,
        DisbandDeadlineTimeNew, MemberCapacityNew] =
        calc_contribution_daily(NowTime, ContributionGetNextTime, Contribution, ContributionDaily, ContributionThreshold,
            Level, DisbandDeadlineTime, MemberCapacity),
    if  % 如果收取了日建设则更新数据库
        (State > 0) ->
            SQLData = [ContributionNew, ContributionGetNextTimeNew, ContributionDailyNew, ContributionThresholdNew,
                LevelNew, DisbandDeadlineTimeNew, MemberCapacityNew, Id],
