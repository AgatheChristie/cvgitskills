...
add_online_goods(GoodsId) ->
    case get_goods_from_db(GoodsId) of
        {ok, []} ->     %% 物品不存在
            ok;
        {ok, [GoodsInfo]} ->
...
%% 检查内容（限制500汉字）
check_content_is_finished(Content) ->
...
check_mail(NameList, Title, Content, GoodsId, Coin, Silver) ->
    case check_title(Title) of  %% 检查标题合法性
        true ->
            case check_content_is_finished(Content) of  %% 检查内容合法性
                true ->
                    F = fun(Item) ->
...
%% 查询物品信息
%% @spec get_goods_from_db(GoodsId) -> GoodsInfo
