...
quality_upgrade(PlayerStatus, GoodsStatus, GoodsInfo, StoneInfo, GoodsQualityRule) ->
    ...
    %% 扣掉品质石
    {ok, NewStatus, NewStoneNum} = lib_goods:delete_goods_by_num(GoodsStatus, StoneInfo, 1),
    %% 更新物品状态
    Ram = util:rand(1, 100),
    case Ratio >= Ram of
        %% 升级成功
        true ->
            NewQuality = upgrade_quality_success(GoodsInfo),
            (catch log:log_quality_up(PlayerStatus, GoodsInfo, GoodsInfo#goods.quality_fail, StoneInfo#goods.goods_id, Cost, 1));
        %% 升级失败
        false ->
            [NewQuality, Quality_fail] = upgrade_quality_fail(GoodsInfo),
            (catch log:log_quality_up(PlayerStatus, GoodsInfo, Quality_fail, StoneInfo#goods.goods_id, Cost, 0))
    end,
    %% 更新装备属性
    AttributeInfo = goods_util:get_attribute_by_goods_and_player(GoodsInfo#goods.player_id, GoodsInfo#goods.id, 3),
    case is_record(AttributeInfo, goods_attribute) of
        ...
        true ->
            ...
            lib_goods:modify_goods_attribute(AttributeInfo, Effect);
        %% 原来没有品质属性，则新增
...
%% 更新品质成功，品质+1
upgrade_quality_success(GoodsInfo) ->
...
%% 更新品质失败，品质-1
upgrade_quality_fail(GoodsInfo) ->
...
quality_backout(PlayerStatus, GoodsStatus, GoodsInfo, GoodsBackoutRuleList) ->
    ...
    case is_record(RuleInfo, ets_goods_quality_backout) of
        ...
        true ->
            ...
            GoodsTypeInfo = goods_util:get_goods_type_by_id(RuleInfo#ets_goods_quality_backout.goods_id),
            case is_record(GoodsTypeInfo, ets_goods_type) of
                true when GoodsNum > 0 ->
                    %% 生成新品质石
                    {ok, NewStatus} = backout_quality_success(GoodsStatus, GoodsTypeInfo, GoodsNum),
...
%% 装备品质石拆除成功
backout_quality_success(GoodsStatus, GoodsTypeInfo, GoodsNum) ->
    NewInfo = goods_util:transform_goods_type_to_goods(GoodsTypeInfo),
    lib_goods:give_goods_to_player(GoodsStatus, GoodsTypeInfo, GoodsNum, NewInfo).
...
strengthen(PlayerStatus, GoodsStatus, GoodsInfo, StoneInfo, RuneInfo, GoodsStrengthenRule) ->
    ...
    NewPlayerStatus = lib_goods:cost_money(PlayerStatus, Cost, coin),
    %% 扣掉强化石
    {ok, NewStatus1, NewStoneNum} = lib_goods:delete_goods_by_num(GoodsStatus, StoneInfo, 1),
    %% 扣掉保底符
    {ok, NewStatus2, _} = lib_goods:delete_goods_by_num(NewStatus1, RuneInfo, 1),
    NewRuneNum = 0,
    %% 更新物品状态
    Ram = util:rand(1, 100),
    case Ratio >= Ram of
        %% 强化成功
        true ->
            NewStrengthen = strengthen_goods_success(GoodsInfo, StoneInfo),
        ...
        %% 强化失败
        false ->
            [NewStrengthen, Stren_fail] = strengthen_goods_fail(GoodsInfo, RuneInfo#goods.id),
            (catch log:log_stren(PlayerStatus, GoodsInfo, Stren_fail, StoneInfo#goods.goods_id, RuneInfo#goods.goods_id, Cost, 0))
    end,
    %% 更新装备属性
    AttributeInfo = goods_util:get_attribute_by_goods_and_player(GoodsInfo#goods.player_id, GoodsInfo#goods.id, 2),
    case is_record(AttributeInfo, goods_attribute) of
        %% 如果降为0级，则删除
...
%% 强化成功，强化等级+1
strengthen_goods_success(GoodsInfo, StoneInfo) ->
...
%% 强化失败，有保底符的话等级-1，没有则等级归零
strengthen_goods_fail(GoodsInfo, RuneId) ->
...
hole(PlayerStatus, Status, GoodsInfo, RuneInfo) ->
    ...
    NewPlayerStatus = lib_goods:cost_money(PlayerStatus, Cost, coin),
    %% 扣掉打孔石
    {ok, NewStatus, NewRuneNum} = lib_goods:delete_goods_by_num(Status, RuneInfo, 1),
    %% 更新物品状态
...
compose(PlayerStatus, Status, StoneList, RuneInfo, GoodsComposeRule) ->
    ...
    F = fun([StoneInfo, StoneNum], Status1) ->
            {ok, NewStatus, _} = lib_goods:delete_goods_by_num(Status1, StoneInfo, StoneNum),
            NewStatus
         end,
    ...
    GoodsTypeInfo = goods_util:get_goods_type_by_id(GoodsComposeRule#ets_goods_compose.new_id),
    case Ratio >= Ram of
        %% 合成成功
        true when is_record(GoodsTypeInfo, ets_goods_type) ->
            {ok, NewStatus3} = compose_success(NewStatus2, GoodsTypeInfo, 1),
...
compose_success(GoodsStatus, GoodsTypeInfo, GoodsNum) ->
    NewInfo = goods_util:transform_goods_type_to_goods(GoodsTypeInfo),
    lib_goods:give_goods_to_player(GoodsStatus, GoodsTypeInfo, GoodsNum, NewInfo).
...
inlay(PlayerStatus, Status, GoodsInfo, StoneInfo, TotalRuneNum, NewRuneList, GoodsInlayRule) ->
    ...
    NewPlayerStatus = lib_goods:cost_money(PlayerStatus, Cost, coin),
    %% 扣掉宝石
    {ok, NewStatus1, _} = lib_goods:delete_goods_by_num(Status, StoneInfo, 1),
    %% 扣掉幸运符
    F = fun([RuneInfo, RuneNum], Status1) ->
            {ok, NewStatus, _} = lib_goods:delete_goods_by_num(Status1, RuneInfo, RuneNum),
            NewStatus
         end,
    ...
    case Ratio >= Ram of
        %% 镶嵌成功
        true ->
            inlay_success(GoodsInfo, StoneInfo),
...
backout(PlayerStatus, Status, GoodsInfo, RuneInfo) ->
   %% 花费铜钱
    ...
    F = fun(StoneId, [Status1, L]) ->
            ...
            case is_record(GoodsTypeInfo, ets_goods_type) of
                true ->
                    {ok, NewStatus1} = backout_success(Status1, GoodsTypeInfo),
                    GoodsList = goods_util:get_goods_list_by_type_id(Status1#goods_status.player_id, StoneId, GoodsTypeInfo#ets_goods_type.bind, 4),
                    NewL = L ++ GoodsList,
                    [NewStatus1, NewL];
                false ->
                    [Status1, L]
    ...
    %% 扣掉幸运符
    {ok, NewStatus1, _} = lib_goods:delete_goods_by_num(NewStatus, RuneInfo, 1),
    %% 更新物品状态
...
backout_success(GoodsStatus, GoodsTypeInfo) ->
    NewInfo = goods_util:transform_goods_type_to_goods(GoodsTypeInfo),
    lib_goods:give_goods_to_player(GoodsStatus, GoodsTypeInfo, 1, NewInfo).
...
wash(PlayerStatus, Status, GoodsInfo, RuneInfo, AddAttributeRuleList) ->
    ...
    {ok, NewStatus, NewRuneNum} = lib_goods:delete_goods_by_num(Status, RuneInfo, 1),
    %% 根据物品品质，决定洗练出来的属性条数
    [_, RuleInfoList] = lists:foldl(fun handle_wash_rule/2, [AddAttributeRuleList, []], lists:seq(1, GoodsInfo#goods.color)),
...
%% 洗附加属性，随机机率，逐个匹配
handle_wash_rule(_, [TotalRuleList, ResultRuleList]) ->
    %% 随机选取一条属性
    FinalRule = util:rand(TotalRuleList, #ets_goods_attribute_rule.ratio),
    RestRuleList = lists:keydelete(FinalRule#ets_goods_attribute_rule.id, #ets_goods_attribute_rule.id, TotalRuleList),
    [RestRuleList, [FinalRule | ResultRuleList]].