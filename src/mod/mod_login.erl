...
logout(Status) ->
    ...
    case Status#player_status.equip_attrit > 0 of
        true -> (catch lib_goods:wear_down_equip(Status, Status#player_status.equip_attrit));
        false -> void
    end,
...
login_success(Player, Pid, Socket, LastLoginTime) ->
  [Id, Accid, Accname, Nickname, Scene, X, Y, Sex, Career, Lv, Hp, Hp_lim, Mp, Mp_lim, Att, Def, Hit, Dodge, Crit, Ten,
      Coin, Bcoin, Cell_num, Mount, Exp, Guild_id, Guild_name, Guild_position, Gold, Silver, Forza, Agile, Wit, Realm,
    ...
    %% 装备属性加成
    [Hp2, Mp2, Att2, Def2, Hit2, Dodge2, Crit2, Ten2] = goods_util:get_equip_attribute(Id, Equip, GoodsStatus#goods_status.equip_suit_list),
    %% 获取技能
    ...
    %% 初始化任务
    lib_task:load_player_task_from_db(PlayerStatus),
    %% todo: 默认触发第一个任务，目前先放到这里，以后会放到创建角色
    lib_task:trigger_task(10100, PlayerStatus),
    %%初始化ets_rela_set
