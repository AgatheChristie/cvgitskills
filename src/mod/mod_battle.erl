...
%%战斗
handle_call({'BATTLE', [Aer, Der, SkillId]} , _FROM, State) ->
    if
        is_record(Aer, player_status) ->
            ...
            {reply, Return, State2};
        true ->
            [Return, State1] = start([Aer, Der, SkillId, State]),
            {reply, Return, State1}
    end;

%% 发动辅助技能
handle_call({assist_skill, [Aer, Der, SkillId]} , _FROM, State) ->
    State1 = use_assist_skill([Aer, Der, SkillId, State]),
    {reply, ok, State1};
...
attack([Aer, Der, SkillId], [AerInit, State]) ->
    {SkillId1, Slv1} = case lists:keyfind(SkillId, 1, Aer#battle_status.skill) of
        false ->
            {0, 0};
        {_, Lv} ->
            {SkillId, Lv}
    end,
    case skill(Aer, Der, SkillId1, Slv1, State) of
        att_area_out -> %% 攻击距离有误
            BData = [
        ...
        false -> %%技能使用有误
            ...
            save(Aer, [AerInit, State]);
        [{Aer1, X, Y, State1 , DefList}, SkillId2] ->
            send_msg([Aer1#battle_status.id, Aer1#battle_status.hp, Aer1#battle_status.mp, SkillId2, X, Y, DefList, Aer1#battle_status.sign, Aer1#battle_status.scene]),
...
%%发送消息
send_msg([Id, Hp, Mp, SkillId, X, Y, DefList, Sign, Q]) when Sign == 1 -> % 怪攻击
    {ok, BinData} = pt_20:write(20003, [Id, Hp, Mp, SkillId, X, Y, DefList]),
    lib_send:send_to_scene(Q, BinData);
send_msg([Id, Hp, Mp, SkillId, X, Y, DefList, Sign, Q]) when Sign == 2 -> % 人攻击
    {ok, BinData} = pt_20:write(20001, [Id, Hp, Mp, SkillId, X, Y, DefList]),
    lib_send:send_to_scene(Q, BinData).

%%发送辅助技能信息
send_assist_msg(Id, SkillId, MP, Q, List)->
    {ok, BinData} = pt_20:write(20006, [Id, SkillId, MP, List]),
    lib_send:send_to_scene(Q, BinData).

%%---------------技能--------------
%% 使用主动技能
%% @param Aer, Der: #battle_status
skill(Aer, Der, SkillId, Lv, State) ->
    case data_skill:get(SkillId, Lv) of
        ...
        SkillData ->
            %% 判断上次放技能时间
            case skill_use_fr(SkillId, SkillData#ets_skill.cd, State) of
                [true, State1] ->
                    %% 判断MP是否足够
                                    ...
                                    case SkillData#ets_skill.type of
                                        1 -> %% 主动
                                            ...
                                            case SkillData#ets_skill.mod of
                                                2 ->
                                                    [double_active_skill(Aer1, Der, SkillData, State1), SkillId];
                                                _ ->
                                                    [single_active_skill(Aer1, Der, SkillData, State1), SkillId]
                                            end;
                                        _ -> %% 技能出错
                                            false
                                    end;
...
%% 使用辅助技能
use_assist_skill([Aer, Der, SkillId, State]) ->
    {SkillId1, Slv1} = case lists:keyfind(SkillId, 1, Aer#player_status.skill) of
        false ->
            {0, 0};
        {_, Lv} ->
            {SkillId, Lv}
    ...
    case data_skill:get(SkillId1, Slv1) of  %% 先判断技能是否存在
        [] ->
            skip;
        SkillData ->
            %% 判断上次放技能时间
                        ...
                        false ->
                            ...
                            case SkillData#ets_skill.obj == 1 of
                                ...
                                false -> %以他人为目标
                                    ...
                                    case Der=={} orelse check_attarea([AttArea, Aer#player_status.x, Aer#player_status.y],
                                        [Der#player_status.x, Der#player_status.y]) of
                                        true -> %% 合理攻击范围内
                                            Aer1 = Aer#player_status{mp = Aer#player_status.mp - MpOut},
...
double_active_skill(Aer, Der, SkillData, State) ->
    ...
    case Der#battle_status.id == Aer2#battle_status.id of
        true ->
            All = AllUser ++ AllMon;
        false ->
            My = [[Der#battle_status.id, Der#battle_status.att, Der#battle_status.def, Der#battle_status.hit,
                Der#battle_status.dodge, Der#battle_status.crit, Der#battle_status.ten, Der#battle_status.hp,
                Der#battle_status.mp, Der#battle_status.sid, Der#battle_status.sign, Der#battle_status.battle_status]],
...
%% @return [Hpb 防守方血量, Mpb 防守方魔力, Hurt 造成的伤害, Status 攻击结果]
...
%% 技能触发频率
skill_use_fr(SkillId, Cd, State) ->
    Time = util:longunixtime(),
    case lists:keyfind(SkillId, 1, State#state.last_skill_time) of
        false ->
            State1 = State#state{last_skill_time = [{SkillId, Time}] ++ State#state.last_skill_time},
            [true, State1];
        {_, Time0} ->
            case Time0 + Cd < Time  of
                true ->
                    LastSkillTime = lists:keydelete(SkillId, 1, State#state.last_skill_time),
                    State1 = State#state{last_skill_time = [{SkillId, Time}] ++ LastSkillTime},
                    [true, State1];
                false ->
                    [false, State] 
            end
