...
init([PlayerId,CellNum, Equip]) ->
    ...
    GoodsStatus = #goods_status{ player_id = PlayerId, null_cells = NullCells, equip_current = [0,0,0], equip_suit_list=EquipSuit },
...
handle_call({'info', GoodsId, Location}, _From, GoodsStatus) ->
    GoodsInfo = case Location =:= ?GOODS_LOCATION_STORAGE of
                    true -> goods_util:get_goods_from_db(GoodsId);
                    false -> goods_util:get_goods_from_ets(GoodsId)
                end,
    case is_record(GoodsInfo, goods) of
        true when GoodsInfo#goods.player_id == GoodsStatus#goods_status.player_id ->
            ...
            SuitNum = goods_util:get_suit_num(GoodsStatus#goods_status.equip_suit_list, GoodsInfo#goods.suit_id),
...
handle_call({'info_other', Owner, GoodsId}, _From, GoodsStatus) ->
    Info = goods_util:get_goods_from_ets(GoodsId),
    GoodsInfo = case is_record(Info, goods) of
                    true -> Info;
                    false -> goods_util:get_goods_from_db(GoodsId)
                end,
    case is_record(GoodsInfo, goods) of
        true ->
            IsOnline = lib_player:is_online(Owner),
            case goods_util:has_attribute(GoodsInfo) of
                true when IsOnline =:= true ->
                    AttributeList = goods_util:get_goods_attribute_list(Owner, GoodsId);
                true ->
                    AttributeList = goods_util:get_offline_goods_attribute_list(Owner, GoodsId);
                false ->
                    AttributeList = []
            end,
            SuitNum = goods_util:get_suit_num(GoodsStatus#goods_status.equip_suit_list, GoodsInfo#goods.suit_id),
...
handle_call({'list', PlayerStatus, Location}, _From, GoodsStatus) ->
    case Location > 0 of
        %% 装备
        true when Location =< ?GOODS_LOCATION_THIRD ->
            NewLocation = Location,
            CellNum = 12,
            EquipList = goods_util:get_goods_list_by_type_and_location(PlayerStatus#player_status.id, 10, NewLocation),
            MountList = goods_util:get_goods_list_by_type_and_location(PlayerStatus#player_status.id, 31, ?GOODS_LOCATION_FIRST),
            List = EquipList ++ MountList;
        true ->
            NewLocation = Location,
            CellNum = case Location =:= ?GOODS_LOCATION_STORAGE of
                          true ->
                              245;  %% 仓库
                          false ->
                              PlayerStatus#player_status.cell_num
                      end,
            List = goods_util:get_goods_list_by_location(PlayerStatus#player_status.id, NewLocation);
        %% 当前装备
        false ->
...
handle_call({'list_other', PlayerId}, _From, GoodsStatus) ->
    case lib_player:is_online(PlayerId) of
        ...
        true ->
            ...
            EquipList = goods_util:get_goods_list_by_type_and_location(PlayerId, 10, Player#ets_online.equip),
            MountList = goods_util:get_goods_list_by_type_and_location(PlayerId, 31, ?GOODS_LOCATION_FIRST),
            List = EquipList ++ MountList,
...
handle_call({'make_list'}, _From, GoodsStatus) ->
    EquipList = goods_util:get_goods_list_by_type_and_location(GoodsStatus#goods_status.player_id, 10, ?GOODS_LOCATION_BAG),
    {reply, EquipList, GoodsStatus};

%% 取商店物品列表
handle_call({'shop', ShopType, ShopSubtype}, _From, GoodsStatus) ->
    ShopList = goods_util:get_shop_goods_list(ShopType, ShopSubtype),
    {reply, ShopList, GoodsStatus};

%%购买物品
handle_call({'pay', PlayerStatus, GoodsTypeId, GoodsNum, ShopType}, _From, GoodsStatus) ->
    case check_buy(PlayerStatus, GoodsStatus, GoodsTypeId, GoodsNum, ShopType) of
        {fail, Res} ->
            {reply, [PlayerStatus, Res, []], GoodsStatus};
        {ok, GoodsTypeInfo, Cost, PriceType} ->
            ...
            case (catch lib_goods:buy_goods(GoodsStatus, GoodsTypeInfo, HasGoodsList, GoodsNum)) of
                {ok, NewStatus} ->
                    NewPlayerStatus = lib_goods:cost_money(PlayerStatus, Cost, PriceType),
                    NewGoodsList = goods_util:get_goods_list_by_type_id(PlayerStatus#player_status.id, GoodsTypeId, GoodsTypeInfo#ets_goods_type.bind, ?GOODS_LOCATION_BAG),
                    lib_task:task_event(buy_equip, {GoodsTypeId}, PlayerStatus#player_status.id),
        ...
        {ok, GoodsInfo, Location, NewCell} ->
            case (catch lib_goods:equip_goods(PlayerStatus, GoodsStatus, GoodsInfo, Location, NewCell)) of
                {ok, NewPlayerStatus, NewStatus, OldGoodsInfo, Effect2} ->
                    lib_task:task_event(equip, {GoodsInfo#goods.goods_id}, GoodsInfo#goods.player_id),
...
%%删除多个物品
handle_call({'delete_goods_by_num', GoodsId, GoodsNum}, _From, GoodsStatus) ->
    GoodsInfo = goods_util:get_goods_from_ets(GoodsId),
    if
        %% 物品不存在
        ...
        true ->
            case (catch lib_goods:delete_goods_by_num(GoodsStatus, GoodsInfo, GoodsNum)) of
                {ok, NewStatus, NewNum} ->
                     lib_player:refresh_client(GoodsStatus#goods_status.player_id, 2),
                     {reply, [1, NewNum], NewStatus};
                 Error ->
                     ?DEBUG("mod_goods delete_goods_by_num:~p", [Error]),
...
handle_call({'delete_more', GoodsTypeId, GoodsNum}, _From, GoodsStatus) ->
    GoodsList = goods_util:get_goods_list_by_type_id(GoodsStatus#goods_status.player_id, GoodsTypeId, ?GOODS_LOCATION_BAG),
    ...
    if 
        ...
        TotalNum < GoodsNum ->
            {reply, 3, GoodsStatus};
        true ->
            case (catch lib_goods:delete_from_goods_list_by_num(GoodsStatus, GoodsList, GoodsNum)) of
                {ok, NewStatus} ->
                     lib_player:refresh_client(GoodsStatus#goods_status.player_id, 2),
                     {reply, 1, NewStatus};
                 Error ->
                     ?DEBUG("mod_goods delete_from_goods_list_by_num:~p", [Error]),
                     {reply, 0, GoodsStatus}
            end
    end;
...
handle_call({'throw_more', GoodsTypeList}, _From, GoodsStatus) ->
    case (catch goods_util:list_handle(fun lib_goods:delete_goods_by_type/2, GoodsStatus, GoodsTypeList)) of
        {ok, NewStatus} ->
...
handle_call({'attrit', PlayerStatus, UseNum}, _From, GoodsStatus) ->
    ...
    db_sql:execute(Sql3),
    %% 获取所有装备
    EquipList = goods_util:get_goods_list_by_type_and_location(PlayerStatus#player_status.id, 10, PlayerStatus#player_status.equip),
    %% 新的装备耐久
    [_, NewEquipList] = lists:foldl(fun lib_goods:wear_down_equip/2, [UseNum, []], EquipList),
    %% 广播耐久更新
    lib_player:refresh_client(PlayerStatus#player_status.id, 5),
    %% 人物属性更新
    case length(NewEquipList) > 0 of
        true ->
            %% 重新计算并更新人物属性
            EquipSuitList = goods_util:count_equip_suit_list(PlayerStatus#player_status.id, PlayerStatus#player_status.equip),
            ...
            %% 检查武器、衣服
            [NewStatus2, _] = goods_util:count_current_equip_by_list(NewEquipList, [NewStatus, off]),
            NewPlayerStatus2 = NewPlayerStatus#player_status{ equip_attrit=0, equip_current=NewStatus2#goods_status.equip_current },
            %% 广播
            {ok, BinData} = pt_12:write(12015, [NewPlayerStatus#player_status.id, NewPlayerStatus#player_status.hp, NewPlayerStatus#player_status.hp_lim, NewEquipList]),
            ...
            %% 通知客户端
            {ok, BinData1} = pt_15:write(15032, NewEquipList),
...
handle_call({'mend_list', PlayerStatus}, _From, GoodsStatus) ->
    %% 查找有耐久度的装备
    List = goods_util:get_mend_goods_list(GoodsStatus#goods_status.player_id, PlayerStatus#player_status.equip),
    F = fun(GoodsInfo) ->
        %% 取最大使用次数
        UseNum = goods_util:get_goods_max_use_num(GoodsInfo#goods.attrition),
...
%% 切换装备套
handle_call({'change_equip', PlayerStatus, EquipLocal}, _From, GoodsStatus) ->
    case check_change_equip(PlayerStatus, EquipLocal) of
        {fail, Res} ->
            {reply, [PlayerStatus, Res, []], GoodsStatus};
        ok ->
            case (catch lib_goods:change_equip(PlayerStatus, GoodsStatus, EquipLocal)) of
...
handle_call({'clean', PlayerStatus}, _From, GoodsStatus) ->
    %% 查询背包物品列表
    GoodsList = goods_util:get_goods_list_by_location(GoodsStatus#goods_status.player_id, ?GOODS_LOCATION_BAG),
    %% 按物品类型ID排序
    GoodsList1 = goods_util:sort(GoodsList, goods_id),
    %% 整理
    [Num, _] = lists:foldl(fun lib_goods:clean_bag/2, [1, {}], GoodsList1),
    %% 重新计算
    NewGoodsList = goods_util:get_goods_list_by_location(GoodsStatus#goods_status.player_id, ?GOODS_LOCATION_BAG),
...
handle_call({'mon_drop', PlayerStatus, MonStatus}, _From, GoodsStatus) ->
    ...
    %% 随机一条掉落数量规则
    NumRule = lib_goods:rand_drop_num_rule(MonId),
    case is_record(NumRule, ets_goods_drop_num) of
        true when NumRule#ets_goods_drop_num.drop_num > 0 ->
            [_TotalRatio, RuleList] = lib_goods:get_drop_rule_list(PlayerId, MonId),
            case length(RuleList) > 0 of
                true ->
                    DropNum = NumRule#ets_goods_drop_num.drop_num,
                    [NewGoodsStatus, _, _, DropGoodsList, TaskResult] = lists:foldl(fun lib_goods:mon_drop/2, [GoodsStatus, RuleList, [], []], lists:seq(1, DropNum)),
                    case length(TaskResult) > 0 of
                        true ->
                            lib_task:task_event(item, TaskResult, PlayerId),
                            lib_player:refresh_client(PlayerId, 2);
                        false -> skip
                    end,
                    case length(DropGoodsList) > 0 of
                        %% 如果有掉落，则插入ETS
                        true ->
                            ExpireTime = util:unixtime() + 60,
                            DropInfo = #ets_goods_drop{ id=RealMonId, player_id=PlayerId, team_id=TeamId, drop_goods=DropGoodsList, expire_time=ExpireTime },
...
%% Local Function
check_buy(PlayerStatus, GoodsStatus, GoodsTypeId, GoodsNum, ShopType) ->
            ...
            case length(GoodsStatus#goods_status.null_cells) < CellNum of
                ...
                false ->
                    ...
                    case goods_util:is_enough_money(PlayerStatus, Cost, PriceType) of
                        %% 金额不足
                        false -> {fail, 3};
                        true -> {ok, GoodsTypeInfo, HasGoodsList, Cost, PriceType}
                    end
            end
    end.
...
check_moveout_bag(GoodsStatus, GoodsId, GoodsNum) ->
    GoodsInfo = goods_util:get_goods_from_db(GoodsId),
    if
        %% 物品不存在
        is_record(GoodsInfo, goods) =:= false ->
            {fail, 2};
...
check_quality_upgrade(PlayerStatus, GoodsId, StoneId) ->
    GoodsInfo = goods_util:get_goods_from_ets(GoodsId),
    StoneInfo = goods_util:get_goods_from_ets(StoneId),
    if
        %% 物品不存在
...
check_quality_backout(GoodsStatus, GoodsId) ->
    ...
    if
        ...
        true ->
            ...
            GoodsBackoutRuleList = goods_util:get_goods_list_from_ets(?ETS_GOODS_QUALITY_BACKOUT, Pattern),
            if
                %% 装备品质石拆除规则不存在
...
check_strengthen(PlayerStatus, GoodsId, StoneId, RuneId) ->
    ...
    StoneInfo = goods_util:get_goods_from_ets(StoneId),
    case RuneId > 0 of
        true -> RuneInfo = goods_util:get_goods_from_ets(RuneId);
...
check_hole(PlayerStatus, GoodsId, RuneId) ->
    Cost = 100,
    GoodsInfo = goods_util:get_goods_from_ets(GoodsId),
    RuneInfo = goods_util:get_goods_from_ets(RuneId),
    if
        %% 物品不存在
...
check_wash(PlayerStatus, GoodsId, RuneId) ->
    ...
    if
        ...
        true ->
            ...
            AddAttributeRuleList = goods_util:get_goods_list_from_ets(?ETS_GOODS_ATTRIBUTE_RULE, Pattern),
            if
                %% 装备附加属性规则不存在
            ...
            case lists:keyfind(GoodsTypeId, 1, DropInfo#ets_goods_drop.drop_goods) of
                ...
                GoodsInfo ->
                    ...
                    if is_record(GoodsTypeInfo, ets_goods_type) =:= false ->
                            {fail, 4};
                        true ->
                            GoodsList = goods_util:get_goods_list_by_type_id(GoodsStatus#goods_status.player_id, GoodsTypeId, GoodsTypeInfo#ets_goods_type.bind, ?GOODS_LOCATION_BAG),
