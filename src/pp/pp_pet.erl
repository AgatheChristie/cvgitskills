...
handle(41001, Status, [GoodsId, GoodsUseNum]) ->
    [Result, PeTaskId, PetName] = mod_pet:incubate_pet(Status, [GoodsId, GoodsUseNum]),
    {ok, BinData} = pt_41:write(41001, [Result, PeTaskId, PetName]),
...
%% 宠物放生
%% -----------------------------------------------------------------
handle(41002, Status, [PeTaskId]) ->
    Result = mod_pet:free_pet(Status, [PeTaskId]),
    {ok, BinData} = pt_41:write(41002, [Result, PeTaskId]),
    lib_send:send_one(Status#player_status.socket, BinData),
    ok;
...
%% 宠物改名
%% -----------------------------------------------------------------
handle(41003, Status, [PeTaskId, PetName]) ->
    [Result, MoneyUseFlag, MoneyLeft] = mod_pet:rename_pet(Status, [PeTaskId, PetName]),
    if  % 改名成功且需要钱
        ((Result == 1) and (MoneyUseFlag == 1)) ->
            % 发送回应
            {ok, BinData} = pt_41:write(41003, [Result, PeTaskId, lib_pet:make_sure_binary(PetName), MoneyUseFlag, MoneyLeft]),
            lib_send:send_one(Status#player_status.socket, BinData),
            % 更新金币
...
%% 宠物出战
%% -----------------------------------------------------------------
handle(41004, Status, [PeTaskId, FightIconPos]) ->
    [Result, IconPos, ForzaUse, WitUse, AgileUse] = mod_pet:fighting_pet(Status, [PeTaskId, FightIconPos]),
    if  Result == 1 ->
            % 发送回应
            {ok, BinData} = pt_41:write(41004, [Result, PeTaskId, IconPos]),
            lib_send:send_one(Status#player_status.socket, BinData),
            % 角色属性加点
